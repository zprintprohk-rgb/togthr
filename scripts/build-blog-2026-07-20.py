...
# -*- coding: utf-8 -*-
"""
Generate the per-slug page.tsx for the 2026-07-20 daily SEO post.
Topic: the day the unedited sentence becomes ordinary.
Slides into the 7/13-7/16 arc as the unremarkable middle: the day the
practice stops being a practice, and the box, the sentence, the pet,
and the morning read all just become part of the day.
...
# ──────────────────────────────────────────────────────────────────────
# 8-locale bodies — culturally adapted, NOT mechanical translation.
# Each locale has: intro, 5 sections, cta, 4 FAQs, 5 internal links.
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
# Render the page.tsx file
# ──────────────────────────────────────────────────────────────────────

def render_body(b):
    out = []
    out.append("    intro: " + repr(b["intro"]) + ",")
    out.append("    sections: [")
    for s in b["sections"]:
        out.append("      { h: " + repr(s["h"]) + ", p: " + repr(s["p"]) + " },")
    out.append("    ],")
    out.append("    cta: " + repr(b["cta"]) + ",")
    out.append("    faqs: [")
    for f in b["faqs"]:
        out.append("      { q: " + repr(f["q"]) + ", a: " + repr(f["a"]) + " },")
    out.append("    ],")
    out.append("    links: [")
    for l in b["links"]:
        out.append("      { href: " + repr(l["href"]) + ", label: " + repr(l["label"]) + " },")
    out.append("    ],")
    return "\n".join(out)


def render_bodies():
    chunks = ["const BODIES: Record<Locale, Body> = {"]
    for loc in ["en", "zh-cn", "zh-tw", "ja", "ko", "de", "fr", "es"]:
        chunks.append(f"  // ─────────────────────── {loc} ───────────────────────")
        chunks.append(f"  '{loc}': {{")
        chunks.append(render_body(BODIES[loc]))
        chunks.append("  },")
        chunks.append("")
    chunks.append("};")
    return "\n".join(chunks)
...
TEMPLATE = r"""// src/app/[locale]/blog/__SLUG__/page.tsx
//
// Per-slug real content page for the __DATE__ daily SEO post.
...
def main():
    out_dir = r"F:\CloudDreamerApp\togthr\src\app\[locale]\blog\the-day-the-unedited-sentence-becomes-ordinary"
    out_path = os.path.join(out_dir, "page.tsx")
    body = TEMPLATE.replace("__SLUG__", SLUG).replace("__DATE__", POST_DATE).replace("__BODIES__", render_bodies())
    ...
    for loc, b in BODIES.items():
        wc = len(b["intro"].split())
        sc = sum(len(s["p"].split()) for s in b["sections"])
        fc = sum(len(f["q"].split()) + len(f["a"].split()) for f in b["faqs"])
        print(f"  {loc:7s} intro={wc:4d}w sections={sc:4d}w faqs={fc:4d}w total={wc+sc+fc:4d}w")
