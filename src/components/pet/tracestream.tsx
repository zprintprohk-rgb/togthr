...
interface TraceEvent {
  id: string; event_type: 'feed' | 'touch' | 'signal'; actor_name: string; created_at: string
}
const EVENT_EMOJI: Record<string, string> = { feed: '🥣', touch: '🫳', signal: '💕' }
...
export default function TraceStream({ coupleId }: { coupleId: string }) {
  ...
  useEffect(() => {
    ...
    supabase.from('events').select('id, event_type, metadata, created_at, profiles!events_actor_id_fkey(display_name)')
      .eq('couple_id', coupleId).order('created_at', { ascending: false }).limit(20)
      .then(({ data }) => {
        if (data) setEvents(data.map((e: Record<string, unknown>) => ({
          id: e.id as string, event_type: e.event_type as TraceEvent['event_type'],
    ...
    <section aria-label="Partner trace" className="mt-6 rounded-2xl border border-zinc-700/40 bg-zinc-900/40 p-5">
      ...
      <ul className="space-y-2">{events.map(e => (
        <li key={e.id} className="flex items-center gap-2 text-sm text-zinc-400">
          <span>{EVENT_EMOJI[e.event_type]}</span> <span className="text-zinc-200">{e.actor_name}</span> <span>·</span>
          <time dateTime={e.created_at} className="text-zinc-500">{new Date(e.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time>
        </li>
      ))}</ul>
    </section>
  )
