...
// Three zero-text buttons to send silent signals to partner.
// Writes to Supabase events table (event_type='signal', metadata.signal_type).
...
export default function SignalButtons({
  ...
  async function sendSignal(type: SignalType) {
    ...
    const { error } = await supabase.from('events').insert({
      ...
      actor_id: userId,
      event_type: 'signal',
      metadata: { signal_type: type },
    })
    if (!error) {
      setSent(type)
      trackGa4Event('signal_send', { signal_type: type, locale })
    ...
    <div className="mt-4 flex gap-2">
      {SIGNALS.map(({ type, emoji, labelKey }) => (
        ...
        >
          ...
          {sent === type && <span className="text-xs text-pink-400">✓</span>}
        </button>
      ))}
    </div>
  )
